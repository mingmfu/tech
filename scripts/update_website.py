...
def update_website():
    ...
    # 检查是否有AI学术分类
    ai_articles = data['categories'][1]['articles'] if len(data['categories']) > 1 else []
    ...
    # 生成AI热点卡片 (显示20条)
    print("🔥 生成AI热点卡片...")
    hot_cards = '\n\n'.join([generate_hot_card(article, i) for i, article in enumerate(hot_articles[:20])])
    ...
    print(f"\n✅ 网站更新完成!")
    print(f"   📰 AI热点: {len(hot_articles[:20])} 条")
